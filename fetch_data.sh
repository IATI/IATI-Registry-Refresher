...
do
  for url_line in `cat $f`; do
    ...
    wget --restrict-file-names=nocontrol --tries=3 --read-timeout=30 --dns-timeout=10 --connect-timeout=10 -U "IATI-Data-Snappshotter" "$url" -O data/`basename $f`/$package_name
